...
const marketData = require('./crex24-markets.json');
...
var Trader = function(config) {
  ...
  this.crex24 = new Crex24.Client(this.key, this.secret);
    ...
    success: data => {
      ...
      if(includes(data, ['Please complete the security check to proceed.']))
        return next(new Error(
          ...
          'As such Gekko Broker cannot access Crex24.'
        ));
...
Trader.prototype.getPortfolio = function(callback) {
  ...
  this.crex24.get('user/accounts').then(handle.success).catch(handle.failure);
};
...
Trader.prototype.getOpenOrders = function(callback) {
  ...
  this.crex24.get('user/orders').then(handle.success).catch(handle.failure);
};
...
Trader.prototype.addOrder = function(type, amount, price, callback) {
  ...
  const handle = this.processResponse(this.addOrder, args, (err, res) => {
    ...
    if(!res.data.id) {
      console.log(new Date, 'Crex24 ERROR! CREATED ORDER BUT NO ID', res);
    }
...
Trader.prototype.getOrder = function(order, callback) {
  ...
  this.crex24.get('user/orders/' + order).then(handle.success).catch(handle.failure);
};
...
Trader.prototype.checkOrder = function(order, callback) {
  ...
  const handle = this.processResponse(this.checkOrder, args, (err, res) => {
    ...
    // https://docs.coinfalcon.com/#list-orders << This line was from the original source
    // I will leave it until I know why this comment was here.  I need to find the Crex24
    // equivalent URL
...
Trader.prototype.cancelOrder = function(order, callback) {
  ...
  this.crex24.delete('user/orders/' + order).then(handle.success).catch(handle.failure);
};
...
Trader.prototype.getTrades = function(since, callback, descending) {
  ...
  this.crex24.get(url).then(success).catch(failure);
};
